package com.seena.leetcode.july;
...
public class SearchSuggestionSystem {
    ...
    private static List<List<String>> suggestedProducts(final String[] products, final String searchWord) {
        ...
        for (int i = 1; i <= searchWord.length(); i++) {
            ...
            for (String product : products) {
                if (product.startsWith(sub) && tmp.size() < 3) {
                    tmp.add(product);
                }
            }
            res.add(tmp);
